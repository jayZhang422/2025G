...
UARTLite 用 16 倍波特率采样，Vivado 2020.2 官方 RTL 将整数分频值取为
`round(Fclk / (16 * baud))`。本工程 `Fclk=100 MHz`、`baud=115200`，理想分频值
为 `54.253`，实际取 `54`，所以实际波特率约为 `115740.741`，误差约 `+0.469%`。
波特率越高，整数分频值越小，分频值每变化 1 所代表的相对步进越大，因此可选
波特率更“粗”：同为 100 MHz 时，9600 baud 的实际误差约 `+0.006%`，而
921600 baud 只能取分频 7，实际约 `892857.143` baud，误差约 `-3.118%`。
因此不能只看屏幕支持的最高波特率，还必须核对 FPGA 时钟下的整数分频误差；
当前 115200 已完成双向板测并保留充足容差。
...
- page2：`x0` 为 Upp（mV），`x1` 为真 RMS（mV），二者固定显示 3 位小数；
  PS 将 mV 值乘以 1000 后写入 `.val`。`n0` 为基频（kHz）。
- page3：`x0..x2` 为频率，`t0..t2` 为正弦峰值幅度并固定显示 3 位小数，
  `t7` 为第三行标签。
...
PL、XDC、BD 和 IP 参数没有变化，因此继续使用冻结 release 中的 bit、XSA 和
`ps7_init.tcl`。PS 源码已合入队长最终标定、`addt` 波形传输、频谱刷新修复和
三位小数显示；对应 ELF 单独放在 `artifacts/current/`，不要再使用冻结 release
中的旧 ELF。
...
| `artifacts/current/hmi_candidate_app.elf` | 2,223,044 | `5021C9ACA3C277A134650C7F4F42C1AADDD8DF15CD0EF198CDE778BCC224187E` |
| `ps7_init.tcl` | 34,511 | `3C4A9FF99EC83AB9C4A8CB98B21C3952AF484E809E050AA075FB151642BCB74D` |
| `26G.HMI` | 7,743,543 | `E8BA47E75D6E21D494C0E016C71AB861D3B4863A7DF87CB1ABE34B88AEFF8BFC` |

XSA 内嵌 bit 的 SHA-256 与独立 bit 完全一致。

验证边界：Vivado/Vitis 2020.2 构建成功，WNS `+0.752 ns`、WHS `+0.035 ns`、
DRC 0 Error。2026-07-31 板测已确认最终标定数值、时域 `1<->3` 周期重复切换、
五轮双谱线显示、重复幅频按钮和约 `390~420 ms` 显示事务均正常，且无 UART
溢出或协议失步。其后只把 `Upp/Urms` 和频域幅值改为三位小数；该缩放已通过
host 回归与完整 Vitis 链接，新 HMI 的 `x0/x1` 小数位也已作二进制属性核对，
但三位小数外观仍需烧入新版 HMI 后做一次最终目视确认。继承设计仍有非 UART
CDC Critical，不能宣称整个系统 CDC clean。
