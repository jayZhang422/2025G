                                               ...
                                               uint8_t byte)
{
    ...
    if (hmi_transfer_parser_feed(
        ...
        uint8_t marker_bit = pl_hmi_uart_marker_bit(marker);

        if (marker == HMI_SERIAL_BUFFER_OVERFLOW) {
            uart->serial_buffer_overflow = 1;
        } else {
            uart->pending_transfer_markers |= marker_bit;
        }
    }
}

static int pl_hmi_uart_drain_events(pl_hmi_uart_t *uart)
{
    uint8_t byte;
                              ...
                              size_t sample_count)
{
    ...
    if (command_length <= 0 || (size_t)command_length >= sizeof(command) ||
        pl_hmi_uart_drain_events(uart) != XST_SUCCESS) {
        return XST_FAILURE;
    }
    ...
    if (pl_hmi_uart_send_bytes(
            uart, samples, sample_count) != XST_SUCCESS ||
        pl_hmi_uart_wait_transfer_marker(
                           ...
                           hmi_event_frame_t *frame)
{
    ...
    (void)pl_hmi_uart_drain_events(uart);
    if (uart->pending_count != 0U) {
        *frame = uart->pending_events[uart->pending_head];
        uart->pending_head = (uint8_t)(
            (uart->pending_head + 1U) % PL_HMI_UART_PENDING_EVENTS);
