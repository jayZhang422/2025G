...
static int g26_hmi_set_text_thousandths(pl_hmi_uart_t *uart,
                                        const char *object_name,
                                        u32 value_thousandths)
{
    return g26_hmi_send_format(
        uart, "%s.txt=\"%u.%03u\"", object_name,
        (unsigned int)(value_thousandths / 1000U),
        (unsigned int)(value_thousandths % 1000U));
}
...
static int g26_hmi_flush_component_updates(g26_hmi_state_t *state)
{
...
static int g26_hmi_confirm_current_page(g26_hmi_state_t *state,
                                        u8 expected_page)
{
    uint8_t page;
...
static int g26_hmi_draw_spectrum_lines(g26_hmi_state_t *state)
{
    ...
    for (component = 0U; component < result->component_count; component++) {
        ...
        if (g26_hmi_draw_spectrum_line(state, x, top) != XST_SUCCESS) {
            return XST_FAILURE;
        }
    }
...
static int g26_hmi_render_page3(g26_hmi_state_t *state)
{
    ...
    if (g26_hmi_flush_component_updates(state) != XST_SUCCESS) {
        return XST_FAILURE;
    }
    if (g26_hmi_draw_spectrum_lines(state) != XST_SUCCESS) {
        return XST_FAILURE;
    }
    return g26_hmi_confirm_current_page(state, G26_HMI_PAGE_SPECTRUM);
}
...
static int g26_hmi_show_page3_amplitudes(g26_hmi_state_t *state)
{
    ...
    for (component = 0U; component < G26_SIGNAL_MAX_COMPONENTS;
        ...
        if (g26_hmi_set_visible(
                &state->uart, amplitude_object,
                component < result->component_count) != XST_SUCCESS) {
            return XST_FAILURE;
        }
    }
    if (g26_hmi_flush_component_updates(state) != XST_SUCCESS) {
...
static int g26_hmi_show_page3_amplitudes_locked(g26_hmi_state_t *state)
{
    int status;
    ...
    status = g26_hmi_show_page3_amplitudes(state);
    if (g26_hmi_set_page_touch_enabled(state, 1) != XST_SUCCESS) {
        status = XST_FAILURE;
    }
    return status;
...
static int g26_hmi_handle_page3_event(g26_hmi_state_t *state, u8 command)
{
    ...
    if (command == G26_HMI_COMMAND_SHOW_AMPLITUDE) {
        ...
        if (state->session.show_amplitudes) {
            return XST_SUCCESS;
        }
        state->session.show_amplitudes = 1;
        if (state->session.snapshot_valid) {
            status = g26_hmi_show_page3_amplitudes_locked(state);
    ...
    const g26_measurement_completion_t *completion)
{
    ...
    status = g26_hmi_set_page_touch_enabled(state, 0);
    if (status == XST_SUCCESS) {
