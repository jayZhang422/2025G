...
static int valid_curve(const dac_vpp_calibration_curve_t *curve)
{
    size_t i;

    if (curve == NULL || curve->count < 2U ||
        curve->count > DAC_VPP_CALIBRATION_MAX_POINTS) {
        return 0;
    }
                              ...
                              float *vpp)
{
    ...
    for (i = 1U; i < curve->count; ++i) {
        const dac_vpp_calibration_point_t *lo = &curve->points[i - 1U];
        const dac_vpp_calibration_point_t *hi = &curve->points[i];
