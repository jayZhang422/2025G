...
## 2026-07-23 PS 运行时底座状态

- `app_runtime_t` 已建立 DMA、DDS、按键和 FFT 实例的运行时边界；初始化顺序为 DDS/按键/DMA/CMSIS FFT。
