...
static int test_transfer_marker_parser(void)
{
    static const uint8_t stream[] = {
        0xFEU, 0xFFU, 0x00U,
        0xAAU, 0x55U, 0x02U, 0x11U, 0xFFU,
        HMI_SERIAL_BUFFER_OVERFLOW, 0xFFU, 0xFFU, 0xFFU,
        HMI_TRANSFER_READY, 0xFFU, 0xFFU, 0xFFU,
        HMI_TRANSFER_DONE, 0xFFU, 0xFFU, 0xFFU
    };
    static const uint8_t expected[] = {
        HMI_SERIAL_BUFFER_OVERFLOW,
        HMI_TRANSFER_READY,
        HMI_TRANSFER_DONE
    };
    hmi_transfer_parser_t parser;
    uint8_t marker = 0U;
    size_t index;
    size_t marker_count = 0U;

    hmi_transfer_parser_init(&parser);
    for (index = 0U; index < sizeof(stream); index++) {
        if (hmi_transfer_parser_feed(
                &parser, stream[index], &marker) != 0) {
            CHECK(marker_count < sizeof(expected));
            CHECK(marker == expected[marker_count]);
            marker_count++;
        }
    }
    CHECK(marker_count == sizeof(expected));
...
static int test_contract_and_numeric_mapping(void)
{
    ...
    CHECK(g26_hmi_millivolts_thousandths(5.0f) == 5000U);
    CHECK(g26_hmi_millivolts_thousandths(49.8714f) == 49871U);
    CHECK(g26_hmi_millivolts_thousandths(174.4805f) == 174481U);
    return 0;
}
...
int main(void)
{
    ...
    CHECK(test_transfer_marker_parser() == 0);
